...
def regdata_fun(request):
    ...
    User_pswd = request.POST['txtUserPassword']
...
def logdata_fun(request):
    ...
    if user1 is not None:
        if user1.is_superuser:
            ...
            return render(request,'login.css',{'data':'User is not a superuser'})
    else:
        return render(request, 'login.css',{'data':"enter proper username and password"})
